...
void genotype_small_inv(inversion_t* inv, open_samFile_t* bam_file, IntervalTree<ext_read_t*>& candidate_reads_for_extension_itree, 
    ...
    auto alt_better_reads_consistent = gen_consensus_and_find_consistent_seqs_subset(alt_seq, alt_better_seqs, alt_better_seqs_isrc, alt_consensus_seq, alt_avg_score, alt_stddev_score, alt_is_exact_read);
    auto ref_better_reads_consistent = gen_consensus_and_find_consistent_seqs_subset(ref_seq, ref_better_seqs, ref_better_seqs_isrc, ref_consensus_seq, ref_avg_score, ref_stddev_score, ref_is_exact_read);
    ...
    if (alt_consensus_seq.length() >= 2*config.min_clip_len) {
        ...
        extend_consensus_to_left(alt_consensus, candidate_reads_for_extension_itree, sv_start-stats.max_is, sv_start, contig_len, config.high_confidence_mapq, stats, mateseqs_w_mapq_chr);
        extend_consensus_to_right(alt_consensus, candidate_reads_for_extension_itree, sv_end, sv_end+stats.max_is, contig_len, config.high_confidence_mapq, stats, mateseqs_w_mapq_chr);
        ...
        delete[] alt_seq;
        alt_start = std::max(hts_pos_t(0), sv_start-stats.max_is);
        alt_lf_len = sv_start-alt_start;
        alt_end = std::min(sv_end+stats.max_is, contig_len);
        alt_rf_len = alt_end-sv_end;
        ...
        delete[] ref_seq;
        ref_start = std::max(hts_pos_t(0), sv_start-stats.max_is);
        ref_end = std::min(sv_end+stats.max_is, contig_len);
        ...
        // align to ref
        aligner.Align(alt_consensus_seq.c_str(), ref_seq, ref_len, filter_with_pos_and_cigar, &ref_aln, 0);
        ...
        hts_pos_t alt_lf_len = sv_start-alt_start;
    ...
    set_bp_consensus_info(inv->sample_info.alt_bp1.reads_info, alt_better_seqs.size(), alt_better_reads_consistent, alt_is_exact_read, alt_avg_score, alt_stddev_score);
    set_bp_consensus_info(inv->sample_info.ref_bp1.reads_info, ref_better_seqs.size(), ref_better_reads_consistent, ref_is_exact_read, ref_avg_score, ref_stddev_score);
...
void genotype_large_inv(inversion_t* inv, open_samFile_t* bam_file, IntervalTree<ext_read_t*>& candidate_reads_for_extension_itree, 
    ...
    auto alt_bp1_better_reads_consistent = gen_consensus_and_find_consistent_seqs_subset(alt_bp1_seq, alt_bp1_better_reads, alt_bp1_better_reads_isrc, alt_bp1_consensus_seq, alt_bp1_avg_score, alt_bp1_stddev_score, alt_bp1_is_exact_read);
    delete[] alt_bp1_seq;
    auto alt_bp2_better_reads_consistent = gen_consensus_and_find_consistent_seqs_subset(alt_bp2_seq, alt_bp2_better_reads, alt_bp2_better_reads_isrc, alt_bp2_consensus_seq, alt_bp2_avg_score, alt_bp2_stddev_score, alt_bp2_is_exact_read);
    delete[] alt_bp2_seq;

    char* ref_bp1_seq = new char[ref_bp1_len+1];
    strncpy(ref_bp1_seq, contig_seq+ref_bp1_start, ref_bp1_len);
    ref_bp1_seq[ref_bp1_len] = 0;
    auto ref_bp1_better_reads_consistent = gen_consensus_and_find_consistent_seqs_subset(ref_bp1_seq, ref_bp1_better_reads, ref_bp1_better_reads_isrc, ref_bp1_consensus_seq, ref_bp1_avg_score, ref_bp1_stddev_score, ref_bp1_is_exact_read);
    delete[] ref_bp1_seq;

    char* ref_bp2_seq = new char[ref_bp2_len+1];
    strncpy(ref_bp2_seq, contig_seq+ref_bp2_start, ref_bp2_len);
    ref_bp2_seq[ref_bp2_len] = 0;
    auto ref_bp2_better_reads_consistent = gen_consensus_and_find_consistent_seqs_subset(ref_bp2_seq, ref_bp2_better_reads, ref_bp2_better_reads_isrc, ref_bp2_consensus_seq, ref_bp2_avg_score, ref_bp2_stddev_score, ref_bp2_is_exact_read);
    delete[] ref_bp2_seq;
    ...
    if (alt_bp1_consensus_seq.length() >= 2*config.min_clip_len) {
        ...
        extend_consensus_to_left(alt_bp1_consensus, candidate_reads_for_extension_itree, sv_start-stats.max_is, sv_start, contig_len, config.high_confidence_mapq, stats, mateseqs_w_mapq_chr);
        extend_consensus_to_right(alt_bp1_consensus, candidate_reads_for_extension_itree, sv_start, sv_start+stats.max_is, contig_len, config.high_confidence_mapq, stats, mateseqs_w_mapq_chr);
    ...
    if (alt_bp2_consensus_seq.length() >= 2*config.min_clip_len) {
        ...
        extend_consensus_to_left(alt_bp2_consensus, candidate_reads_for_extension_itree, sv_end-stats.max_is, sv_end, contig_len, config.high_confidence_mapq, stats, mateseqs_w_mapq_chr);
        extend_consensus_to_right(alt_bp2_consensus, candidate_reads_for_extension_itree, sv_end, sv_end+stats.max_is, contig_len, config.high_confidence_mapq, stats, mateseqs_w_mapq_chr);
    ...
    alt1_aln.Clear();
    if (alt_bp1_consensus_seq.length() >= 2*config.min_clip_len) {
        hts_pos_t ref_bp1_start = sv_start-alt_bp1_consensus_seq.length();
        if (ref_bp1_start < 0) ref_bp1_start = 0;
        hts_pos_t ref_bp1_end = sv_start+alt_bp1_consensus_seq.length();
        ...
        hts_pos_t alt_bp1_start = std::max(hts_pos_t(0), hts_pos_t(sv_start-alt_bp1_consensus_seq.length()));
        hts_pos_t alt_bp1_end = std::min(hts_pos_t(sv_start+alt_bp1_consensus_seq.length()), contig_len);
        hts_pos_t alt_bp1_len = alt_bp1_end-alt_bp1_start;
        alt_bp1_seq = new char[alt_bp1_len+1];
        strncpy(alt_bp1_seq, contig_seq+alt_bp1_start, sv_start-alt_bp1_start);
        ...
        strncpy(alt_bp1_seq+(sv_start-alt_bp1_start), ext_inv_suffix_rc, ext_inv_border_len);
        int extra_len = alt_bp1_consensus_seq.length() - ext_inv_border_len;
        if (extra_len > 0) {
            if (sv_end + extra_len > contig_len) extra_len = contig_len - sv_end;
            strncpy(alt_bp1_seq+(sv_start-alt_bp1_start)+ext_inv_border_len, contig_seq+sv_end, extra_len);
        }
        alt_bp1_seq[alt_bp1_len] = 0;

        aligner.Align(alt_bp1_consensus_seq.c_str(), alt_bp1_seq, alt_bp1_len, filter_with_pos_and_cigar, &alt1_aln, 0);
        delete[] ext_inv_suffix_rc;
        delete[] alt_bp1_seq;

        hts_pos_t alt_lf_len = sv_start - alt_bp1_start;
    ...
    alt2_aln.Clear();
    if (alt_bp2_consensus_seq.length() >= 2*config.min_clip_len) {
        hts_pos_t ref_bp2_start = sv_end-alt_bp2_consensus_seq.length();
        if (ref_bp2_start < 0) ref_bp2_start = 0;
        hts_pos_t ref_bp2_end = sv_end+alt_bp2_consensus_seq.length();
        ...
        hts_pos_t alt_bp2_start = std::max(hts_pos_t(0), hts_pos_t(sv_end-alt_bp2_consensus_seq.length()));
        hts_pos_t alt_bp2_end = std::min(hts_pos_t(sv_end+alt_bp2_consensus_seq.length()), contig_len);
        ...
        if (extra_len > 0) {
            if (extra_len > sv_start) extra_len = sv_start;
            strncpy(alt_bp2_seq, contig_seq+sv_start-extra_len, extra_len);
        } else extra_len = 0;
        ...
        strncpy(alt_bp2_seq+extra_len+ext_inv_border_len, contig_seq+sv_end, alt_bp2_end-sv_end);
        alt_bp2_seq[alt_bp2_len] = 0;

        aligner.Align(alt_bp2_consensus_seq.c_str(), alt_bp2_seq, alt_bp2_len, filter_with_pos_and_cigar, &alt2_aln, 0);
        delete[] ext_inv_prefix_rc;
        delete[] alt_bp2_seq;

        hts_pos_t alt_rf_len = alt_bp2_end - sv_end;
    ...
    set_bp_consensus_info(inv->sample_info.alt_bp1.reads_info, alt_bp1_better_reads.size(), alt_bp1_better_reads_consistent, alt_bp1_is_exact_read, alt_bp1_avg_score, alt_bp1_stddev_score);
    set_bp_consensus_info(inv->sample_info.alt_bp2.reads_info, alt_bp2_better_reads.size(), alt_bp2_better_reads_consistent, alt_bp2_is_exact_read, alt_bp2_avg_score, alt_bp2_stddev_score);
    set_bp_consensus_info(inv->sample_info.ref_bp1.reads_info, ref_bp1_better_reads.size(), ref_bp1_better_reads_consistent, ref_bp1_is_exact_read, ref_bp1_avg_score, ref_bp1_stddev_score);
    set_bp_consensus_info(inv->sample_info.ref_bp2.reads_info, ref_bp2_better_reads.size(), ref_bp2_better_reads_consistent, ref_bp2_is_exact_read, ref_bp2_avg_score, ref_bp2_stddev_score);
}
