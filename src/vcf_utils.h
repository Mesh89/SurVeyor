...
void add_fmt_tags(bcf_hdr_t* hdr) {
	...
	bcf_hdr_remove(hdr, BCF_HL_INFO, "REF1_HP_LEN");
	const char* ref1_hp_len_tag = "##INFO=<ID=REF1_HP_LEN,Number=1,Type=Integer,Description=\"Length of the longest homopolymer in the breakpoint 1 reference allele sequence.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, ref1_hp_len_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_INFO, "REF2_HP_LEN");
	const char* ref2_hp_len_tag = "##INFO=<ID=REF2_HP_LEN,Number=1,Type=Integer,Description=\"Length of the longest homopolymer in the breakpoint 2 reference allele sequence.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, ref2_hp_len_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_INFO, "ALT1_HP_LEN");
	const char* alt1_hp_len_tag = "##INFO=<ID=ALT1_HP_LEN,Number=1,Type=Integer,Description=\"Length of the longest homopolymer in the breakpoint 1 alternate allele sequence.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, alt1_hp_len_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_INFO, "ALT2_HP_LEN");
	const char* alt2_hp_len_tag = "##INFO=<ID=ALT2_HP_LEN,Number=1,Type=Integer,Description=\"Length of the longest homopolymer in the breakpoint 2 alternate allele sequence.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, alt2_hp_len_tag, &len));
	...
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, xaas_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_FMT, "XAAS2");
	const char* xaas2_tag = "##FORMAT=<ID=XAAS2,Number=1,Type=Integer,Description=\"Edit distance between the extended alternative allele consensus for the second breakpoint and the original alternative allele (the reference with the SV applied).\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, xaas2_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_FMT, "XARS");
	const char* xars_tag = "##FORMAT=<ID=XARS,Number=1,Type=Integer,Description=\"Score of the alignment between the extended alternative allele consensus and the reference.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, xars_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_FMT, "XARS2");
	const char* xars2_tag = "##FORMAT=<ID=XARS2,Number=1,Type=Integer,Description=\"Score of the alignment between the extended alternative allele consensus for the second breakpoint and the reference.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, xars2_tag, &len));
	...
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, xass_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_FMT, "XASS2");
	const char* xass2_tag = "##FORMAT=<ID=XASS2,Number=2,Type=Integer,Description=\"How much of the extended alternative allele aligns to the left and right, respectively, of the second breakpoint (only for insertions).\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, xass2_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_FMT, "XASSC");
	const char* xassc_tag = "##FORMAT=<ID=XASSC,Number=2,Type=Integer,Description=\"Score of the left half and right half of the extended alternative allele consensus.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, xassc_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_FMT, "XASSCIA");
	const char* xasscia_tag = "##FORMAT=<ID=XASSCIA,Number=2,Type=Integer,Description=\"Score of the left half and right half of the extended alternative allele consensus for the first breakpoint, when allowed to map to the reference independently.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, xasscia_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_FMT, "XASSC2");
	const char* xassc2_tag = "##FORMAT=<ID=XASSC2,Number=2,Type=Integer,Description=\"Score of the left half and right half of the extended alternative allele consensus for the second breakpoint (only for insertions).\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, xassc2_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_FMT, "XASSC2IA");
	const char* xassc2ia_tag = "##FORMAT=<ID=XASSC2IA,Number=2,Type=Integer,Description=\"Score of the left half and right half of the extended alternative allele consensus for the second breakpoint, when allowed to map to the reference independently.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, xassc2ia_tag, &len));
	...
	bcf_hdr_remove(hdr, BCF_HL_FMT, "RR1HPMODE");
	const char* rr1m_tag = "##FORMAT=<ID=RR1HPMODE,Number=1,Type=Integer,Description=\"Mode of homopolymer length among all reads assigned to breakpoint 1 of the REF allele.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, rr1m_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_FMT, "RR1CHPMODE");
	const char* rr1cm_tag = "##FORMAT=<ID=RR1CHPMODE,Number=1,Type=Integer,Description=\"Mode of homopolymer length among consistent reads assigned to breakpoint 1 of the REF allele.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, rr1cm_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_FMT, "RR1CHPIQR");
	const char* rr1hpiqr_tag = "##FORMAT=<ID=RR1CHPIQR,Number=1,Type=Float,Description=\"Interquartile range of observed homopolymer lengths among good homopolymer reads assigned to breakpoint 1 of the REF allele.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, rr1hpiqr_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_FMT, "RR1CHPmQ");
	const char* rr1hpminmq_tag = "##FORMAT=<ID=RR1CHPmQ,Number=1,Type=Integer,Description=\"Minimum read mapping quality among good homopolymer reads assigned to breakpoint 1 of the REF allele.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, rr1hpminmq_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_FMT, "RR1CHPMQ");
	const char* rr1hpmaxmq_tag = "##FORMAT=<ID=RR1CHPMQ,Number=1,Type=Integer,Description=\"Maximum read mapping quality among good homopolymer reads assigned to breakpoint 1 of the REF allele.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, rr1hpmaxmq_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_FMT, "RR1CHPAQ");
	const char* rr1hpavgmq_tag = "##FORMAT=<ID=RR1CHPAQ,Number=1,Type=Float,Description=\"Average read mapping quality among good homopolymer reads assigned to breakpoint 1 of the REF allele.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, rr1hpavgmq_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_FMT, "RR1CHPSQ");
	const char* rr1hpstdmq_tag = "##FORMAT=<ID=RR1CHPSQ,Number=1,Type=Float,Description=\"Standard deviation of read mapping quality among good homopolymer reads assigned to breakpoint 1 of the REF allele.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, rr1hpstdmq_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_FMT, "RR1HP5PMR");
	const char* rr1hp5pmr_tag = "##FORMAT=<ID=RR1HP5PMR,Number=1,Type=Float,Description=\"Average 5' tail mismatch rate among homopolymer reads assigned to breakpoint 1 of the REF allele.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, rr1hp5pmr_tag, &len));

	bcf_hdr_remove(hdr, BCF_HL_FMT, "RR1HP3PMR");
	const char* rr1hp3pmr_tag = "##FORMAT=<ID=RR1HP3PMR,Number=1,Type=Float,Description=\"Average 3' tail mismatch rate among homopolymer reads assigned to breakpoint 1 of the REF allele.\">";
	bcf_hdr_add_hrec(hdr, bcf_hdr_parse_line(hdr, rr1hp3pmr_tag, &len));
	...
	const char* hopr_tag = "##FORMAT=<ID=HOPR,Number=1,Type=Float,Description=\"Probability of the SV to be homozygous, according to the ML model.\">";
...
std::string get_sv_type(bcf_hdr_t* hdr, bcf1_t* sv) {
    ...
    if (bcf_get_info_string(hdr, sv, "SVTYPE", &data, &len) < 0) {
		...
		if (sv->d.allele[1][0] == '<') {
			...
			if (alt_len < ref_len) return "DEL";
			if (alt_len > ref_len) return "INS";
			return ref_len == 1 ? "SNP" : "MNP";
		}
