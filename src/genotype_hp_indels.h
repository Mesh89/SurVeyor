...
std::vector<hp_allele_cluster_t> cluster_reads_by_cached_association(const std::vector<hp_read_info_t>& hp_read_infos,
    std::vector<int>& hp_run_lens, std::vector<sv_t*>& hp_indels, evidence_map_t* evidence_map) {

    std::vector<hp_allele_cluster_t> clusters;
    for (int allele_idx = 0; allele_idx < hp_indels.size(); allele_idx++) clusters.push_back(hp_allele_cluster_t(allele_idx, hp_run_lens[allele_idx]));
    int ref_allele_idx = hp_indels.size();
    clusters.push_back(hp_allele_cluster_t(ref_allele_idx, hp_run_lens[ref_allele_idx]));
    ...
    for (const hp_read_info_t& hp_read_info : hp_read_infos) {
        ...
        for (const evidence_map_t::read_alt_association_t& association : evidence_map->get_read_alt_associations(hp_read_info.read)) {
            ...
            if (allele_idx >= 0) break;
        }
        if (allele_idx < 0) {
            for (sv_t* hp_indel : hp_indels) {
                if (evidence_map->is_read_ref(hp_read_info.read, hp_indel, 1)) {
                    allele_idx = ref_allele_idx;
                    break;
                }
            }
        }
        if (allele_idx >= 0) clusters[allele_idx].reads.push_back(hp_read_info);
    }
...
struct hp_indel_alignment_targets_t {
    char hp_base;
    std::unique_ptr<char[]> ref_allele;
    int ref_allele_len;
    hts_pair_pos_t ref_allele_hp_range;
    std::vector<std::unique_ptr<char[]>> candidate_ref_alleles, alt_alleles;
    std::vector<int> candidate_ref_allele_lens, alt_allele_lens;
    std::vector<hts_pair_pos_t> candidate_ref_allele_hp_ranges, alt_allele_hp_ranges;
};

hp_indel_alignment_targets_t build_hp_indel_alignment_targets(std::vector<sv_t*>& hp_indels, hts_pair_pos_t ref_hp_range, char* contig_seq, hts_pos_t contig_len, stats_t& stats) {
    hp_indel_alignment_targets_t targets;
    targets.hp_base = get_homopolymer_base(hp_indels[0], contig_seq);
    ...
    targets.ref_allele_len = left_flank_len + ref_hp_len + right_flank_len;

    targets.ref_allele.reset(new char[targets.ref_allele_len + 1]);
    strncpy(targets.ref_allele.get(), contig_seq + alt_start, left_flank_len);
    memset(targets.ref_allele.get() + left_flank_len, targets.hp_base, ref_hp_len);
    strncpy(targets.ref_allele.get() + left_flank_len + ref_hp_len, contig_seq + ref_hp_range.end, right_flank_len);
    targets.ref_allele[targets.ref_allele_len] = '\0';
    ...
    for (sv_t* hp_indel : hp_indels) {
        ...
        memset(candidate_ref_seq.get() + alt_lh_len, targets.hp_base, ref_hp_len);
        strncpy(candidate_ref_seq.get() + alt_lh_len + ref_hp_len, rh_seq, alt_rh_len);
        candidate_ref_seq[candidate_ref_len] = '\0';
        targets.candidate_ref_alleles.push_back(std::move(candidate_ref_seq));
        targets.candidate_ref_allele_lens.push_back(candidate_ref_len);
        targets.candidate_ref_allele_hp_ranges.push_back({alt_lh_len, alt_lh_len + ref_hp_len});
        ...
        memset(alt_seq.get() + alt_lh_len, targets.hp_base, alt_hp_len);
        strncpy(alt_seq.get() + alt_lh_len + alt_hp_len, rh_seq, alt_rh_len);
        alt_seq[alt_len] = '\0';
        targets.alt_alleles.push_back(std::move(alt_seq));
        targets.alt_allele_lens.push_back(alt_len);
        targets.alt_allele_hp_ranges.push_back({alt_lh_len, alt_lh_len + alt_hp_len});
        delete[] lh_seq;
        delete[] rh_seq;
    }
    return targets;
...
void write_aligned_hp_indels_group_read_evidence(std::vector<sv_t*>& hp_indels, hts_pair_pos_t ref_hp_range, open_samFile_t* bam_file, char* contig_seq, hts_pos_t contig_len, stats_t& stats, config_t& config, StripedSmithWaterman::Aligner& aligner, std::unordered_map<std::string, std::pair<std::string, int>>& mateseqs_w_mapq_chr, evidence_logger_t& evidence_logger) {
    ...
    for (const std::shared_ptr<bam1_t>& collected_read : collected_reads) {
        ...
        hp_read_info_t hp_read_info = calculate_hp_read_info(read, ref_hp_range, targets.hp_base, contig_seq, contig_len, targets.ref_allele.get(), targets.ref_allele_len, targets.ref_allele_hp_range, has_no_left_indel, has_no_right_indel, 10, config.max_seq_error);
        if (hp_read_info.tail_3p_len < config.min_clip_len || hp_read_info.tail_5p_len < config.min_clip_len) {
            std::string seq = get_sequence(read);
            for (int i = 0; i < hp_indels.size(); i++) {
                aligner.Align(seq.c_str(), targets.candidate_ref_alleles[i].get(), targets.candidate_ref_allele_lens[i], filter_score_only, &candidate_ref_aln, 0);
                aligner.Align(seq.c_str(), targets.alt_alleles[i].get(), targets.alt_allele_lens[i], filter_default, &alt_aln, 0);
                bool alt_spans_hp = alt_aln.ref_begin <= targets.alt_allele_hp_ranges[i].beg && alt_aln.ref_end >= targets.alt_allele_hp_ranges[i].end - 1;
                ...
                std::vector<int> alt_scores = { alt_aln.sw_score };
                std::vector<int> alt_positions = { alt_aln.ref_begin };
                evidence_logger.log_reads_associations(hp_indels[i]->chr, hp_indels[i]->id, 1, read_v, alt_scores, alt_positions);
            }
            continue;
        }
    ...
    possible_mates_ss << hp_indels[0]->chr << ":" << std::max(hts_pos_t(0), ref_hp_range.beg - stats.max_is) << "-" << std::min(contig_len, ref_hp_range.end + stats.max_is);
    ...
    while (sam_itr_next(bam_file->file, iter, read) >= 0) {
        ...
        aligner.Align(mate_seq.c_str(), targets.ref_allele.get(), targets.ref_allele_len, filter_default, &ref_aln, 0);
        bool aln_as_rev = !bam_is_rev(read);
        if ((!aln_as_rev && get_left_clip_size(ref_aln) > 0) || (aln_as_rev && get_right_clip_size(ref_aln) > 0)) continue;
        ...
        hp_read_info_t hp_read_info = calculate_hp_read_info(ref_aln, mate_seq, targets.ref_allele_hp_range, targets.hp_base, targets.ref_allele.get(), targets.ref_allele_len, aln_as_rev, rescued_read, 10, has_no_left_indel, has_no_right_indel, config.max_seq_error);
    ...
    for (int i = 0; i < hp_read_infos.size(); i++) {
        if (valid_reads_mask[i]) valid_hp_read_infos.push_back(hp_read_infos[i]);
    }
    hp_read_infos.swap(valid_hp_read_infos);

    std::vector<int> hp_run_lens;
    for (sv_t* hp_indel : hp_indels) hp_run_lens.push_back(ref_hp_range.end - ref_hp_range.beg + hp_indel->svlen());
    hp_run_lens.push_back(ref_hp_range.end - ref_hp_range.beg);
    std::vector<hp_allele_cluster_t> clusters = cluster_reads_by_nearest_allele_len(hp_read_infos, targets.alt_alleles, targets.alt_allele_lens, hp_run_lens, aligner);
    std::vector<std::vector<bp_support_read_t>> ref_reads(hp_indels.size());

    for (const hp_allele_cluster_t& allele_cluster : clusters) {
        std::vector<bp_support_read_t> cluster_reads;
        for (const hp_read_info_t& hp_read_info : allele_cluster.reads) cluster_reads.push_back(hp_read_info.read);

        std::vector<int> best_allele_idxs;
        for (int allele_idx = 0; allele_idx < hp_run_lens.size(); allele_idx++) {
            if (hp_run_lens[allele_idx] == allele_cluster.allele_len) best_allele_idxs.push_back(allele_idx);
        }
        ...
        for (int best_allele_idx : best_allele_idxs) {
            if (is_ref_allele) {
                for (int i = 0; i < hp_indels.size(); i++) ref_reads[i].insert(ref_reads[i].end(), cluster_reads.begin(), cluster_reads.end());
            } else {
                std::vector<int> alt_positions;
                std::vector<int> alt_scores = calculate_aln_scores(allele_cluster.reads, targets.alt_alleles[best_allele_idx].get(), targets.alt_allele_lens[best_allele_idx], aligner, &alt_positions);
                evidence_logger.log_reads_associations(hp_indels[best_allele_idx]->chr, hp_indels[best_allele_idx]->id, 1, cluster_reads, alt_scores, alt_positions);
            }
        }
    }

    for (int i = 0; i < hp_indels.size(); i++) evidence_logger.log_ref_reads_associations(hp_indels[i]->chr, hp_indels[i]->id, 1, ref_reads[i]);
}
...
inline void genotype_hp_indels_group(std::vector<sv_t*>& hp_indels, hts_pair_pos_t ref_hp_range, open_samFile_t* bam_file, char* contig_seq, hts_pos_t contig_len,
    ...
    hp_indel_alignment_targets_t alignment_targets = build_hp_indel_alignment_targets(hp_indels, ref_hp_range, contig_seq, contig_len, stats);
    char hp_base = alignment_targets.hp_base;
    std::unique_ptr<char[]>& ref_allele = alignment_targets.ref_allele;
    int ref_allele_len = alignment_targets.ref_allele_len;
    hts_pair_pos_t ref_allele_hp_range = alignment_targets.ref_allele_hp_range;
    std::vector<std::unique_ptr<char[]>>& candidate_ref_alleles = alignment_targets.candidate_ref_alleles;
    std::vector<int>& candidate_ref_allele_lens = alignment_targets.candidate_ref_allele_lens;
    std::vector<hts_pair_pos_t>& candidate_ref_allele_hp_ranges = alignment_targets.candidate_ref_allele_hp_ranges;
    ...
    for (const std::shared_ptr<bam1_t>& collected_read : collected_reads) {
        bam1_t* read = collected_read.get();
        bool assigned_outside_group = reassigns_evidence(evidence_mode);
        for (int i = 0; i < hp_indels.size(); i++) {
             if (!reassigns_evidence(evidence_mode) || !evidence_map->is_read_assigned_to_different_sv(read, hp_indels[i])) {
    ...
    while (sam_itr_next(bam_file->file, iter, read) >= 0) {
        ...
        bool assigned_outside_group = reassigns_evidence(evidence_mode);
        for (sv_t* hp_indel : hp_indels) {
            if (!reassigns_evidence(evidence_mode) || !evidence_map->is_read_assigned_to_different_sv(hp_read_info.read, hp_indel)) {
                assigned_outside_group = false;
                break;
            }
        }
    ...
    std::vector<hp_allele_cluster_t> clusters;
    if (reassigns_evidence(evidence_mode)) clusters = cluster_reads_by_nearest_allele_len_reassign(hp_read_infos, hp_run_lens, hp_indels, evidence_map, config.seed);
    else clusters = cluster_reads_by_cached_association(hp_read_infos, hp_run_lens, hp_indels, evidence_map);
    ...
    for (const hp_allele_cluster_t& allele_cluster : clusters) {
        ...
        if (!reassigns_evidence(evidence_mode)) {
...
inline void genotype_hp_indels(int id, std::string contig_name, char* contig_seq, int contig_len, std::vector<sv_t*> hp_indels,
    ...
    for (auto& kv : hp_indels_by_ref_hp_range) {
        ...
        genotype_hp_indels_group(hp_indels_in_range, ref_hp_range, bam_file, contig_seq, contig_len, stats, config, aligner,
            *mateseqs_w_mapq_chr, evidence_mode, evidence_map);
    }
